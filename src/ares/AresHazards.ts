...
export class _AresHazardPlacement {
  ...
  public static getAllLandSpacesAdjacentToHazards(game: Game): ISpace[] {
    const hazardTiles = game.board.spaces.filter((space) => space.tile && HAZARD_TILES.has(space.tile.tileType));
  ...
  public static getHazardsCount(game: Game): number {
    return game.board.spaces.filter((space) => space.tile && HAZARD_TILES.has(space.tile.tileType)).length;
  }
